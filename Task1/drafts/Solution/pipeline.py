...
# VISUALIZATION
# ==============================================================================
def plot_deviation_3d(pred, actual_df, model_name, output_path):
    """3D surface: deviation (pred - actual) across dates and tenors."""
    from mpl_toolkits.mplot3d import Axes3D
    from matplotlib import cm

    deviation = pred.astype(float) - actual_df.astype(float)

    tenor_idx = np.arange(len(YIELD_TENORS))
    date_idx = np.arange(len(deviation.index))
    X, Y = np.meshgrid(tenor_idx, date_idx)
    Z = deviation.values.astype(float)
    ...
    surf = ax.plot_surface(X, Y, Z, facecolors=colors, edgecolor='grey',
                           linewidth=0.3, alpha=0.9, shade=False)

    mappable = cm.ScalarMappable(cmap=cm.RdBu_r, norm=norm)
    mappable.set_array(Z)
    fig.colorbar(mappable, ax=ax, shrink=0.5, pad=0.1, label='Deviation (pred - actual)')

    ax.set_xticks(tenor_idx)
    ax.set_xticklabels(YIELD_TENORS, fontsize=9, rotation=20)
    ax.set_xlabel('Tenor', fontsize=11, labelpad=10)

    date_labels = [d.strftime('%Y-%m') for d in deviation.index]
    ax.set_yticks(date_idx)
    ax.set_yticklabels(date_labels, fontsize=9)
    ax.set_ylabel('Date', fontsize=11, labelpad=10)

    ax.set_zlabel('Deviation', fontsize=11, labelpad=10)
    ax.set_title(f'{model_name}: Forecast Deviation (2025-04 .. 2025-09)',
...
def main():
    ...
    print("\n[6] 3D deviation plots...")
    results_dir = os.path.join(output_dir, "results")
    os.makedirs(results_dir, exist_ok=True)
    actual_df = yield_df.loc[pred_m1.index, YIELD_TENORS]
    p1 = plot_deviation_3d(pred_m1, actual_df, "M1 (PCA-delta Ridge)",
                           os.path.join(results_dir, "deviation_3d_M1.png"))
    print(f"  3D M1: {p1}")
    p2 = plot_deviation_3d(pred_m2, actual_df, "M2 (70% ElasticNet + 30% PCA-Ridge)",
                           os.path.join(results_dir, "deviation_3d_M2.png"))
    print(f"  3D M2: {p2}")
