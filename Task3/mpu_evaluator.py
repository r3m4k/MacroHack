...
class MPUEvaluator:
    ...
    def plot_oos_timeseries(self,
        ...
        # Граница IS/OOS
        split_date = merged.loc[n_split, 'Date'] \
            if n_split < len(merged) else merged['Date'].iloc[-1]
    ...
    def _merge(self, mpu: pd.Series) -> pd.DataFrame:
        ...
        mpu_df = (mpu.reset_index()
                  .rename(columns={mpu.index.name or 'index': 'Date',
                                   'MPU': 'MPU'}))
        mpu_df.columns = ['Date', 'MPU']
        mpu_df['Date'] = pd.to_datetime(mpu_df['Date'])

        rv_df = (self.rv.reset_index()
                 .rename(columns={self.rv.index.name or 'index': 'Date',
                                  self.rv.name: 'RV'}))
        rv_df.columns  = ['Date', 'RV']
